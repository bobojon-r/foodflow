...
- [x] **Этап 4: Stripe** — интеграция платежей, webhooks, идемпотентность.
...
Сервис оплаты (`payment-service`) интегрирован со Stripe через PaymentIntent API.

### Как работает

```
order.created (Kafka)
    → payment-service создаёт Stripe PaymentIntent
    → Stripe обрабатывает платёж
    → Stripe отправляет webhook → /api/v1/webhooks/stripe
    → payment-service публикует payment.succeeded (Kafka)
    → notification-service уведомляет пользователя
```

### Настройка

1. Зарегистрируйся на [stripe.com](https://stripe.com) (бесплатно, тестовый режим)
2. Dashboard → **Developers** → **API keys** → скопируй **Secret key** (`sk_test_...`)
3. Добавь в `.env`:
   ```env
   STRIPE_SECRET_KEY=sk_test_...
   ```

4. Для webhooks локально — установи [Stripe CLI](https://stripe.com/docs/stripe-cli):
   ```bash
...
# Создай заказ через API → payment-service автоматически создаст PaymentIntent
# В логах увидишь: PaymentIntent pi_xxx status=succeeded
# Придёт webhook → payment.succeeded в Kafka → уведомление в Telegram
```

> **Без `STRIPE_SECRET_KEY`** сервис работает в режиме симуляции — сразу публикует `payment.succeeded` без реального платежа.
